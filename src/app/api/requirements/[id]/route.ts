...
export async function PATCH(req:NextRequest,{params}:{params:Promise<{id:string}>}){
 ...
 if(!access.isAdmin){
  const owns=await db.query("SELECT 1 FROM requirements r JOIN projects p ON p.id=r.project_id WHERE r.id=$1 AND p.access_code_id=$2",[id,access.id]);
  if(!owns.rowCount)return NextResponse.json({error:"条目不存在"},{status:404});
 }
